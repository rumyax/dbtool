...
def db_run(args):
    ...
    run([
        ...
        f'--username={DB[key]['user']}',
        f'--dbname={DB[key]['name']}',
        f'--file={file}' if file else f'--command={command}'
    ], vars={'PGPASSWORD': DB[key]['password']})
...
def main():
    ...
    # run
    parser_run = subparsers.add_parser('run', help='Run SQL file or command on database')
    parser_run.add_argument('--on', dest='target', required=True, help='Target DB')
    parser_run_source = parser_run.add_mutually_exclusive_group(required=True)
    parser_run_source.add_argument('--file', dest='file', help='SQL file to run')
    parser_run_source.add_argument('--command', dest='command', help='SQL command to run')
